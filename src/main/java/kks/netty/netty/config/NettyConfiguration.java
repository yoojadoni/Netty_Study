...
import kks.netty.netty.socket.NettyChannelInitializer;
...
public class NettyConfiguration {
    ...
    public ServerBootstrap serverBootstrap(NettyChannelInitializer nettyChannelInitializer) {
        ...
        b.option(ChannelOption.SO_BACKLOG, backlog); //backlog
    ...
    public NioEventLoopGroup bossGroup() {
        return new NioEventLoopGroup(bossCount); //bossCount
    }

    // worker: boss가 수락한 연결의 트래픽 관리
    @Bean(destroyMethod = "shutdownGracefully")
    public NioEventLoopGroup workerGroup() {
        return new NioEventLoopGroup(workerCount); //workerCount
    }
    ...
    public InetSocketAddress inetSocketAddress() {
        return new InetSocketAddress(port); //host
    }
}
